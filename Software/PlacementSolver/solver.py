                ...
                for k in range (len(row_assign)):
                    if row_assign[k][0]== comp[i][j]:
                        for l in range(1,len(row_assign[k])):
                            assignment_done=0
                            for t in range(4):
                                if bb[int(row_assign[k][l])][t]==0 and int(row_assign[k][l])>(no_rows_on_board/2):
                                    #print(row_assign[k][l]+str(t+1),end=" ")
                                    comp[i][j]=[row_assign[k][l],str(t+1)]
                                    bb[int(row_assign[k][l])][t]=1
                                elif int(row_assign[k][l])<=int(no_rows_on_board/2) and bb[int(row_assign[k][l])][4-t]==0:
                                    #print(row_assign[k][l]+str(5-t),end=" ")
                                    comp[i][j]=[row_assign[k][l],str(5-t)]
                                    bb[int(row_assign[k][l])][4-t]=1
                                else:
                                    continue
                                assignment_done=1
                                break
